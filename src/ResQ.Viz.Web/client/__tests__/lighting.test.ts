...
// Imported rather than duplicated so the assertion below tracks the real
// constant. scene.ts is safe to import for a value: its WebGL work happens in
// the Scene constructor, not at module scope.
import { CASTER_ENVELOPE_M } from '../scene';
...
describe('caster envelope', () => {
    it('measures max terrain height across every preset', () => {
        ...
        // The actual guard. Asserting only finite-and-positive let a preset be
        // retuned to 400 m peaks with both checks still passing, while
        // CASTER_ENVELOPE_M stayed at 260 and the directional light sank below
        // the peaks at low sun elevation — exactly the failure this block
        // describes. If this fires, raise CASTER_ENVELOPE_M to the measured
        // value logged above rather than relaxing the assertion.
        expect(globalMax).toBeLessThanOrEqual(CASTER_ENVELOPE_M);
    });
});
